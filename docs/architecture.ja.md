> Snapshot of `docs/architecture.md` at the point it was translated to
> English. Not maintained going forward — see `docs/architecture.md` for
> the current rules.
...
| `dom/` (`.ts`) | DOM計測・スタイル書き込み・イベント購読(Shadow DOM canvasのマウント/スケール同期、ドラッグジェスチャ、CodeMirrorエディタ同期) | シグナル、IPC、JSX | 数式部分は`domain/`に押し出して`bun test`。残りはIRテスト/e2e |
...
IME合成中のエディタ(CodeMirror)同期、プレビュー枠のiframe上をドラッグ中にカーソルが

...
- 構造操作の保存中にユーザーが別のスライドへ移ると、`reconcileAfterCommit`は
  選択を動かさない(`now`をそのまま返す)ため、開いているスライドの位置は
  コマンドに合わせてずらされない(このtodo以前からの挙動)。保持した状態は
  `shift`でずれるので、選択の位置と食い違う場合があるが、復元時の本文一致
  チェックで別スライドの履歴が付くことはない。
- IMEの変換中にスライドを切り替えると、エディタは差し替わらず前のスライドの
  本文を表示したままになる(このtodo以前からの挙動、`view.composing`)。

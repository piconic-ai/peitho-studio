...
## 追記: 本当の原因はUIスレッドのブロックだった

楽観的遷移の後も、ユーザーの実機確認では「何が変わったか分からない」
という結果だった。原因はフロントの見せ方ではなかった:

- Rustの`open_deck`は同期コマンドで、TauriはこれをUIスレッドで実行
  する。アプリ起動後の初回は約5秒かかる(重いデッキで実測。2回目以降
  は0.1〜0.7秒)。
- その間ウィンドウは再描画されない。DOMはクリック4ms後に
  「Loading deck…」へ切り替わっていたが、ネイティブ録画ではWelcome画面
  が約5秒間止まったまま映っていた。DOMを見るテスト(モック・実機とも)
  はこれを検出できない。
- `open_deck`を`#[tauri::command(async)]`にしたところ、同じ条件の録画
  でクリック直後から「Loading deck…」が描画され、読み込み中ずっと
  表示されることを確認した。

上の「完了」判定は、軽いデッキとDOMベースの確認だけで下したもので
誤りだった。

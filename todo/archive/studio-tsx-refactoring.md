...
- [ ] **Step 20**: `Studio.tsx`を合成ルートに整理(目標200〜300行)。
  ...
  - [x] **Step 20-4**完了。`state/deckStore.ts`(新規)に`deckLifecycle`
        ...
        **これで`state/`層への切り出しは一区切り**——`uiStore`/
        ...
        ステップで行う。
  - [x] **最終**完了。`Studio.tsx`は2293行(開始時)→1117行(60行の
        軽微な整形含む最終値)。**目標の200〜300行には届かなかった**——
        理由は行数不足ではなく、実装して初めて分かった設計上の発見:
        `docs/architecture.md`の5層モデル(domain/state/ipc/dom/
        components)には、「複数のstateストアを横断し、IPCを呼び、
        ときにDOMにも触れる一連の手続き」(`commitChange`/
        `selectSlide`/`addSlide`/`reorderSlides`等、約600行)の
        置き場所が存在しない——`domain/`は不純だから不可、`state/`は
        `ipc/`に依存できないから不可、`ipc/`は薄いラッパーの範囲を
        超えるから不可、`dom/`はIPCを許さないから不可。この発見自体を
        `docs/architecture.md`に新セクション「5層モデルに収まらない
        もの: オーケストレーション」として追記し、恒久的な設計知識
        として残した(選択肢: 新しく「orchestrator」層を作る/
        `state`から`ipc`への依存を許可する、のどちらも今回は選ばず
        現状維持)。
        `CLAUDE.md`の「BarefootJSで踏んだ落とし穴」は、このリファクタ
        リング全体を通じて逐次更新済み(BF044のprops reactivity知見、
        Step 7訂正の反映、`fragmentSignal`の参照先を
        `state/renderStore.ts`に修正、等)——最後に全体を読み返し、
        矛盾や古い参照が残っていないことを確認した。
        全spec/adversarialテスト通過(236 pass)。
        **これでStep 1〜20、Studio.tsxリファクタリング計画は完了。**

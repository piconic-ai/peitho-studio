...
ユーザーの「手軽に試せるなら試したい」を受けて、案Aを #59 に積む PR として
実装した。起動時にバックグラウンドのスレッドで `engine::warm_up` を呼び、
Recent の上位3件を一度レンダリングして結果は捨てる。上の試作と違って短い
サンプルではなく実際のデッキを描くので、そのデッキが使う正規表現がすべて
コンパイル済みになる。
...
| 計測 | 温めなし | 温めあり |
|---|---|---|
| 一時テスト: 重いデッキの `render_source`(Recent 上位3件に含まれる) | 約570ms | 約13ms |
| 一時テスト: 同(含まれない。ハイライタ初期化分だけ速い) | 約570ms | 約290ms |
| 実機: 起動4秒後に Recent をクリック → スライド一覧の DOM 出現 | 615/616ms | 51/56ms |

温め自体は起動直後に約0.6秒(3件、debug)。実機計測は tauri-playwright で、
`PEITHO_STUDIO_NO_WARMUP` 環境変数による一時的な切り替えで比較した
(切り替えはコミットしていない)。
